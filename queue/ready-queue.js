  ...
  function QueueNode(config) {
    ...
    function setStatusTimer() {
      if (!isConnected || (isConnected && !queue.isEmpty())) {
        if (!statusTimer) statusTimer = setInterval(statusOutput, 500);
      } else if (statusTimer) {
        clearInterval(statusTimer);
        statusTimer = null;
      }
    ...
    function statusOutput() {
      ...
      if (!queue.isEmpty() && isConnected) {
        ...
        s = {
          fill: "green",
          shape: "dot",
          text: "Forwarding" + remaining,
        };
      } else if (!isConnected) {
        s = {
          fill: "yellow",
          shape: "ring",
          text: "Storing" + remaining,
        };
      }
      // Only update our status if it has changed
      if (!shallowequal(s, status)) node.status(s);
    ...
    function initialState(msg) {
      if (msg.hasOwnProperty("status")) initStateMsgs.push(msg);
      else initMsgs.push(msg);
    }
    ...
    function processStatus(msg) {
      ...
      var status = msg.status.text
        .toString()
        .replace(/^node-red:common\.status\./, "");
    ...
    queue.on("open", function () {
      ...
      if (initStateMsgs.length > 0) {
        // If we received status messages during initialisation, process them now
        initStateMsgs.forEach(function (m) {
          processStatus(m);
        });
